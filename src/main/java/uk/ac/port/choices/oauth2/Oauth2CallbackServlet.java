    ...
    public void doGet(HttpServletRequest req, HttpServletResponse resp){
        ...
        try{
            ...
            if (req.getSession().getAttribute(LoginServlet.STATE) == null
                    || !req.getParameter(LoginServlet.STATE).equals(req.getSession().getAttribute(LoginServlet.STATE))) {
            ...
            GoogleAuthorizationCodeFlow flow = new GoogleAuthorizationCodeFlow.Builder(
                    ...
                    getServletContext().getInitParameter("auth.clientID"),
                    getServletContext().getInitParameter("auth.clientSecret"),
                    Oauth2CallbackServlet.SCOPES).build();

            TokenResponse tokenResponse =
                    flow.newTokenRequest(req.getParameter("code"))
                            .setRedirectUri(getServletContext().getInitParameter("auth.callback"))
                            .execute();

            req.getSession().setAttribute(Oauth2CallbackServlet.SESSION_TOKEN, tokenResponse.getAccessToken()); // Keep track of the token.
            Credential credential = flow.createAndStoreCredential(tokenResponse, null);
            HttpRequestFactory requestFactory = Oauth2CallbackServlet.HTTP_TRANSPORT.createRequestFactory(credential);

            GenericUrl url = new GenericUrl(Oauth2CallbackServlet.USERINFO_ENDPOINT);      // Make an authenticated request.
            ...
            req.getSession().setAttribute(Oauth2CallbackServlet.SESSION_USER_EMAIL, userIdResult.get("email"));
            req.getSession().setAttribute(Oauth2CallbackServlet.SESSION_USER_ID, userIdResult.get("sub"));
            req.getSession().setAttribute(Oauth2CallbackServlet.SESSION_USER_IMAGE_URL, userIdResult.get("picture"));
            req.getSession().setAttribute(Oauth2CallbackServlet.SESSION_USER_NAME, userIdResult.get("name"));
